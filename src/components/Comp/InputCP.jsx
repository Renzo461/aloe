...
const InputCP = ({ value, label, placeholder, id, className }) => {    
  return (    
    ...
    <div className={`flex items-center ${className}`}>
      <label htmlFor={id} className="flex-1">{label}</label>
      <input type="text" id={id} name={id} placeholder={placeholder} onChange={(event) => { value(event.target.value) }} className='border rounded-lg ml-4 p-1 flex-1' />
    </div>
  )
}
